...
final class KnowledgePageQueryTest extends TestCase
{
    ...
    public function test_public_query_returns_active_claims_and_hides_private_or_unlinked_evidence(): void
    {
        ...
        $sources = new class($sourceId, $privateSourceId) implements SourceRepository {
            ...
            public function findByCanonicalId(string $id): ?Source { return $id === $this->activeId ? new Source($id, 'nhk:source:public', 'Public source', 'catalog', 'https://example.test/source', ['visibility' => 'PUBLIC']) : ($id === $this->privateId ? new Source($id, 'nhk:source:private', 'Private source', 'catalog', null, ['visibility' => 'PRIVATE']) : null); }
