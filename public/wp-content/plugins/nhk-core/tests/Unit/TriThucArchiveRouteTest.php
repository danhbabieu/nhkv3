...
final class TriThucArchiveRouteTest extends TestCase
{
    public function test_tri_thuc_archive_query_targets_the_canonical_published_category_newest_first(): void
    {
        ...
        self::assertStringContainsString("'category__in' => [4]", $routes);
        self::assertStringContainsString("'post_type' => 'post'", $routes);
        self::assertStringContainsString("'post_status' => 'publish'", $routes);
        self::assertStringContainsString("'orderby' => 'date'", $routes);
        self::assertStringContainsString("'order' => 'DESC'", $routes);
        self::assertStringNotContainsString("category_name=tri-thuc'", $routes);
    }
    ...
    public function test_tri_thuc_archive_renders_native_post_cards_with_fallback_and_pagination(): void
    {
        ...
        self::assertStringContainsString('<div class="post-grid">', $index);
        self::assertStringContainsString('$archiveQuery = new WP_Query([', $index);
        self::assertStringContainsString("'category__in' => [4]", $index);
        self::assertStringContainsString("'post_type' => 'post'", $index);
        self::assertStringContainsString("'post_status' => 'publish'", $index);
        self::assertStringContainsString("'orderby' => 'date'", $index);
        self::assertStringContainsString("'order' => 'DESC'", $index);
        self::assertStringContainsString("'paged' => \$archivePage", $index);
        self::assertStringContainsString("'posts_per_page' => get_option('posts_per_page')", $index);
        self::assertStringContainsString('while ($archiveQuery->have_posts())', $index);
        self::assertStringContainsString('$archiveQuery->the_post()', $index);
        self::assertStringContainsString('wp_reset_postdata()', $index);
        self::assertStringContainsString("'total' => (int) \$archiveQuery->max_num_pages", $index);
        self::assertStringContainsString("get_template_part('template-parts/article-card')", $index);
    ...
    public function test_tri_thuc_archive_exposes_the_canonical_heading_and_url(): void
    {
        ...
        self::assertStringContainsString('nhk_v3_public_editorial_label($editorialRoute)', $index);
