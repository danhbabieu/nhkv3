...
final class GraphWpdbIntegrationTest extends TestCase {
    ...
    public function test_raw_graph_rest_route_is_not_public(): void { require_once dirname(__DIR__, 2) . '/nhk-core.php'; do_action('rest_api_init'); $routes = rest_get_server()->get_routes(); $route = ''; foreach (array_keys($routes) as $candidate) { if (str_starts_with($candidate, '/nhk/v1/graph/')) { $route = $candidate; break; } } self::assertNotSame('', $route); $originalUserId = get_current_user_id(); wp_set_current_user(0); try { $permission = $routes[$route][0]['permission_callback'] ?? null; self::assertIsCallable($permission); self::assertFalse($permission(new \WP_REST_Request('GET', $route))); } finally { wp_set_current_user($originalUserId); } }
    ...
    public function test_graph_repository_omits_corrupt_edge_rows(): void
    {
        global $wpdb;
        $postId = wp_insert_post(['post_title' => 'NHK corrupt edge source', 'post_status' => 'draft']);
        $targetId = wp_insert_post(['post_title' => 'NHK corrupt edge target', 'post_status' => 'draft']);
        $repo = new WpdbGraphRepository($wpdb);
        $types = new EndpointTypeRegistry();
        $types->register('wp_post', new WpPostEndpointResolver());
        $service = new GraphService($repo, $types, new PredicateRegistry(), new InMemoryAuditSink());
        $source = new NodeReference('wp_post', get_current_blog_id() . ':' . $postId);
        $target = new NodeReference('wp_post', get_current_blog_id() . ':' . $targetId);
        $edge = $service->create($source, 'about', $target);

        $wpdb->query($wpdb->prepare('UPDATE ' . $wpdb->prefix . 'nhk_graph_edges SET revision=%d WHERE edge_uuid=UNHEX(%s)', 0, str_replace('-', '', $edge->edge_uuid)));

        self::assertNull($repo->findByUuid($edge->edge_uuid));
        self::assertSame([], $repo->outgoing($repo->findNode($source), null, 0, 10, false)['items']);

        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'nhk_graph_edges WHERE edge_uuid=UNHEX(%s)', str_replace('-', '', $edge->edge_uuid)));
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'nhk_graph_nodes WHERE endpoint_key IN (%s,%s)', get_current_blog_id() . ':' . $postId, get_current_blog_id() . ':' . $targetId));
