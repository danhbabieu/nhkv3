...
final class GraphMigrationIdempotencyTest extends TestCase {
    ...
    public function test_authority_migration_up_down_up_is_idempotent(): void { global $wpdb; TestDatabaseGuard::assertDestructiveAllowed((string)DB_NAME); $table=$wpdb->prefix.'nhk_entities'; self::assertSame($table,$wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s',$table))); $wpdb->query("DELETE FROM {$table}"); (new AuthorityMigration002())->up(); (new AuthorityMigration002())->up(); (new AuthorityMigration002())->down(); self::assertSame(0,(int)$wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM information_schema.tables WHERE table_schema=DATABASE() AND table_name=%s',$table))); (new AuthorityMigration002())->up(); self::assertSame($table,$wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s',$table))); }
}
