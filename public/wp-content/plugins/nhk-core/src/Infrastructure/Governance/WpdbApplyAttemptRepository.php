...
final class WpdbApplyAttemptRepository implements ApplyAttemptRepository
{
    ...
    private function row(?array $r):?ApplyAttempt{if(!$r)return null; try{$states=['pending','running','succeeded','failed'];$state=(int)$r['state'];if($state<1||$state>count($states))return null;return new ApplyAttempt(UuidCodec::fromBinary($r['attempt_uuid']),UuidCodec::fromBinary($r['proposal_uuid']??$r['_proposal_uuid']), (int)$r['attempt_no'],$states[$state-1],!empty($r['result_entity_uuid'])?UuidCodec::fromBinary($r['result_entity_uuid']):null,$r['error_code']??null,$r['error_message']??null,$r['started_at']??null,$r['finished_at']??null);}catch(\Throwable){return null;}}
