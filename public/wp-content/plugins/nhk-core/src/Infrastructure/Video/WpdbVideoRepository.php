...
final class WpdbVideoRepository implements VideoRepository
{
    ...
    private function hydrate(?array $row): ?Video
    {
        ...
        try {
            ...
            return new Video(UuidCodec::fromBinary($row['canonical_uuid']), (string) $row['platform'], (string) $row['external_video_id'], (string) $row['canonical_url'], (string) $row['title'], $metadata, $row['thumbnail_media_uuid'] === null ? null : UuidCodec::fromBinary($row['thumbnail_media_uuid']), (int) $row['state'] === 1, (int) $row['revision'], $row['created_at'] ?? null, $row['updated_at'] ?? null);
        } catch (\Throwable) {
            return null;
        }
    }
