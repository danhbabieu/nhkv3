...
final class WpdbAuthorityRepository implements AuthorityRepository {
 ...
 private function row(?array $r):?AuthorityEntity{if(!$r)return null;try{$payload=json_decode((string)($r['payload']??''),true,512,JSON_THROW_ON_ERROR);$state=(int)$r['state'];if(!is_array($payload)||!in_array($state,[0,1],true))return null;return new AuthorityEntity(UuidCodec::fromBinary($r['canonical_uuid']),$r['entity_type'],$r['stable_key'],$r['canonical_name'],(int)$r['schema_version'],$payload,$state===1?AuthorityState::ACTIVE:AuthorityState::RETIRED,(int)$r['revision'],$r['created_at'],$r['updated_at'],$r['retired_at']);}catch(\Throwable){return null;}}
