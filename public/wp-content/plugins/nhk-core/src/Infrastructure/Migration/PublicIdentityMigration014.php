...
final class PublicIdentityMigration014
{
    ...
    public function up(): void
    {
        ...
        dbDelta("CREATE TABLE {$wpdb->prefix}nhk_public_identities (id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, identity_uuid BINARY(16) NOT NULL, owner_kind VARCHAR(64) NOT NULL, owner_uuid BINARY(16) NOT NULL, route_type VARCHAR(64) NOT NULL, current_slug VARCHAR(191) NOT NULL, current_path VARCHAR(255) NOT NULL, collision_scope VARCHAR(191) NOT NULL, route_policy_version VARCHAR(64) NOT NULL, revision INT UNSIGNED NOT NULL DEFAULT 1, idempotency_key VARCHAR(191) NOT NULL, created_at DATETIME(6) NOT NULL, updated_at DATETIME(6) NOT NULL, PRIMARY KEY (id), UNIQUE KEY identity_unique (identity_uuid), UNIQUE KEY owner_unique (owner_kind,owner_uuid,route_type), UNIQUE KEY route_unique (route_type,collision_scope,current_slug), UNIQUE KEY idempotency_unique (idempotency_key)) {$charset}");
        dbDelta("CREATE TABLE {$wpdb->prefix}nhk_public_identity_history (id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, identity_uuid BINARY(16) NOT NULL, route_type VARCHAR(64) NOT NULL, collision_scope VARCHAR(191) NOT NULL, route_path VARCHAR(255) NOT NULL, old_slug VARCHAR(191) NOT NULL, revision INT UNSIGNED NOT NULL, created_at DATETIME(6) NOT NULL, PRIMARY KEY (id), UNIQUE KEY history_route (route_type,collision_scope,route_path), KEY identity_lookup (identity_uuid)) {$charset}");
