...
final class GovernanceMigration003
{
    ...
    public function up(): void
    {
        ...
        if (!$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name=%s AND column_name='dependency_fingerprint'", $p.'nhk_proposals'))) {
            $wpdb->query("ALTER TABLE {$p}nhk_proposals ADD dependency_fingerprint BINARY(32) NOT NULL AFTER fingerprint");
        }
