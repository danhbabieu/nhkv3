...
final class MediaEndpointResolver implements EndpointResolver
{
    ...
    public function supports(string $endpoint_type): bool { return $endpoint_type === 'media'; }

    public function normalize(NodeReference $reference): NodeReference
    {
        if (!$this->supports($reference->endpoint_type) || preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i', $reference->endpoint_key) !== 1) throw new InvalidEndpointReference('Media endpoint key must be UUID.');
        return $reference;
    }
