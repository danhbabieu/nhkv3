...
final class WpdbEvidenceRepository implements EvidenceRepository
{
    ...
    private function hydrate(?array $row): ?Evidence { if (!$row) return null; $metadata = (string) ($row['metadata_json'] ?? ''); if ($metadata === '') { $decodedMetadata = []; } else { try { $decodedMetadata = json_decode($metadata, true, 512, JSON_THROW_ON_ERROR); } catch (\JsonException) { return null; } if (!is_array($decodedMetadata)) return null; } return new Evidence(UuidCodec::fromBinary($row['evidence_uuid']), UuidCodec::fromBinary($row['claim_uuid']), UuidCodec::fromBinary($row['source_uuid']), (string) $row['relation_type'], (string) $row['excerpt'], $row['locator'] === null ? null : (string) $row['locator'], (int) $row['state'] === 1, (int) $row['revision'], $decodedMetadata); }
