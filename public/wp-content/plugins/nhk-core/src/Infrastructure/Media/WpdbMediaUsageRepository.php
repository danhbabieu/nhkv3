...
final class WpdbMediaUsageRepository implements MediaUsageRepository
{
    ...
    private function hydrate(array $row): MediaUsage
    {
        ...
        if (!is_string($mediaUuid) || strlen($mediaUuid) !== 16) throw new MediaException('Media parent not found.');
        return new MediaUsage(UuidCodec::fromBinary($row['usage_uuid']), UuidCodec::fromBinary($mediaUuid), (string) $row['endpoint_type'], (string) $row['endpoint_key'], (string) $row['usage_role'], (int) $row['sort_order']);
    }
