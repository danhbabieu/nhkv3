...
final class WpdbProjectionDependencyIndex implements ProjectionDependencyIndex
{
    ...
    public function __construct(private object $database) { $this->table = $database->prefix . 'nhk_claim_projection_dependencies'; }
    ...
    {
        $rows = $this->database->get_results($this->database->prepare("SELECT dependency_kind AS kind,dependency_id AS id,node_uuid,node_type,section_key,scope,graph_distance,dependency_revision FROM {$this->table} WHERE dependency_kind=%s AND dependency_id=%s", $kind, $id), ARRAY_A);
        return is_array($rows) ? $rows : [];
    }

    public function removeForNode(string $nodeUuid): void { $this->database->query($this->database->prepare("DELETE FROM {$this->table} WHERE node_uuid=%s", $nodeUuid)); }
}
