...
final class AdminPage
{
    ...
    private static function renderProposalComposer(): void
    {
        ...
        echo '<h2>Create governed proposal</h2><p>Soạn lệnh semantic để đưa vào lifecycle Submit → Approve → Controlled Apply.</p><form id="nhk-proposal-composer" class="nhk-proposal-composer"><p><label>Operation <select name="operation"><option value="create">create</option><option value="ingest">ingest</option><option value="relation_create">relation_create</option><option value="rename">rename</option><option value="update">update</option><option value="retire">retire</option><option value="relation_retire">relation_retire</option><option value="reactivate">reactivate</option><option value="relation_reactivate">relation_reactivate</option></select></label> <label>Entity type <select name="entity_type">';
        foreach ($types->all() as $definition) echo '<option value="' . esc_attr($definition->type) . '">' . esc_html($definition->type) . '</option>';
        echo '</select></label></p><p><label>Target UUID / subject ID <input name="target_uuid" size="40" placeholder="UUID entity/edge cho update/retire"></label> <label>Expected revision <input name="expected_revision" type="number" min="1" value="1" size="5"></label></p><p><label>Stable key <input name="stable_key" size="28" placeholder="brand.example"></label> <label>Name <input name="name" size="32" placeholder="Tên canonical"></label></p><p><label>Relation source <input name="source_type" size="12" placeholder="wp_post"></label> <input name="source_key" size="24" placeholder="1:42"> <label>Predicate <input name="predicate" size="18" placeholder="about"></label> <label>Target <input name="target_type" size="12" placeholder="brand"></label> <input name="target_key" size="24" placeholder="UUID"></p><p><label>Entity payload JSON<br><textarea name="entity_payload" rows="4" cols="90" placeholder="{&quot;description&quot;:&quot;...&quot;}">{}</textarea></label></p><p><button class="button button-primary" type="submit">Create proposal</button></p><pre class="nhk-composer-result" aria-live="polite"></pre></form>';
    }

    private static function renderSemanticReadTools(): void
    {
        echo '<h2>Semantic & Graph lookup</h2><p>Tra cứu Media, Video, Knowledge, Source và các cạnh Graph qua read API.</p><form id="nhk-semantic-lookup"><label>Domain <select name="domain"><option value="media">Media</option><option value="video">Video</option><option value="knowledge/claim">Knowledge Claim</option><option value="knowledge/source">Source</option></select></label> <label>Canonical UUID <input name="id" size="40" required></label> <button class="button" type="submit">Tra cứu</button></form><form id="nhk-graph-lookup"><label>Direction <select name="direction"><option value="outgoing">outgoing</option><option value="incoming">incoming</option></select></label> <label>Endpoint type <input name="endpoint_type" value="wp_post" size="14" required></label> <label>Endpoint key <input name="endpoint_key" size="24" placeholder="1:42" required></label> <label>Predicate <input name="predicate" size="16" placeholder="about"></label> <button class="button" type="submit">Xem Graph</button></form><pre id="nhk-semantic-result" class="nhk-governance-result" aria-live="polite"></pre>';
    }
