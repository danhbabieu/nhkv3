...
final class AdminMediaAdapter
{
    ...
    public function detail(Media $media): array
    {
        ...
        return [
            ...
            'usages' => array_map(static fn (MediaUsage $usage): array => ['usage_id' => $usage->usageId, 'media_id' => $usage->mediaId, 'endpoint_type' => $usage->endpointType, 'endpoint_key' => $usage->endpointKey, 'target_type' => $usage->endpointType, 'target_id' => $usage->endpointKey, 'role' => $usage->role, 'placement_key' => $usage->placementKey, 'sort_order' => $usage->sortOrder, 'active' => $usage->activeSlot !== 'retired', 'revision' => $usage->revision, 'selection_source' => $usage->selectionSource, 'selection_policy' => $usage->selectionPolicy, 'active_slot' => $usage->activeSlot, 'alt' => $usage->altText, 'caption' => $usage->caption, 'title' => $usage->title], $usages),
