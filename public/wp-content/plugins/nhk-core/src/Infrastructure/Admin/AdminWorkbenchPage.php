...
final class AdminWorkbenchPage
{
    ...
    public static function renderAutomationPolicy(): void
    {
        ...
        foreach (array_values(array_unique(array_merge(['wp_post'], array_values(array_diff(array_keys($types), ['wp_post', 'media', 'video', 'knowledge', 'source', 'evidence', 'relation']))))) as $targetType) {
    ...
    private static function renderMediaWorkspace(): void
    {
        ...
        echo '<section id="nhk-media-video-manager" class="nhk-admin-panel" data-nhk-video-media-workspace><h2>Video</h2><p>Ưu tiên xử lý Video đang thiếu ảnh đại diện. Ảnh nguồn chỉ là fallback, không phải Media canonical.</p><nav class="nhk-admin-filters" aria-label="Lọc ảnh đại diện Video"><button type="button" class="button is-active" data-nhk-video-filter="all">Tất cả</button><button type="button" class="button" data-nhk-video-filter="representative">Có ảnh đại diện</button><button type="button" class="button" data-nhk-video-filter="source">Đang dùng ảnh nguồn</button><button type="button" class="button" data-nhk-video-filter="missing">Thiếu ảnh đại diện</button></nav><div id="nhk-video-media-list" aria-live="polite"><p class="nhk-admin-empty">Đang tải danh sách Video…</p></div><div id="nhk-video-media-panel" class="nhk-admin-detail" hidden></div></section>';
        echo '<section class="nhk-admin-panel"><h2>Thêm ảnh vào nội dung hoặc entity</h2><p>Upload tạo Media/Asset vật lý; bước dưới đây tạo Proposal MediaUsage. Policy Admin quyết định tự động phê duyệt hay đưa vào hàng đợi hiện hữu.</p><form id="nhk-media-upload-form" enctype="multipart/form-data"><label for="nhk-media-upload-files">Ảnh</label><input id="nhk-media-upload-files" name="files[]" type="file" accept="image/jpeg,image/png,image/gif,image/webp" multiple required><button class="button" type="submit">Upload ảnh</button></form><form id="nhk-media-usage-form" class="nhk-admin-form"><label for="nhk-media-usage-operation">Thao tác</label><select id="nhk-media-usage-operation"><option value="add">Thêm Media</option><option value="replace">Thay Media</option><option value="remove">Gỡ Media</option><option value="representative_bind">Đặt Media đại diện</option></select><label for="nhk-media-usage-media">Media UUID</label><input id="nhk-media-usage-media" required placeholder="UUID từ kết quả upload hoặc tìm kiếm"><label for="nhk-media-usage-target-type">Target type</label><select id="nhk-media-usage-target-type"><option value="wp_post">Bài viết (wp_post)</option><option value="classification">Classification</option><option value="brand">Brand</option><option value="model">Model</option><option value="movement">Movement</option></select><label for="nhk-media-usage-target-id">Target ID / Post ID</label><input id="nhk-media-usage-target-id" required><label for="nhk-media-usage-role">Role</label><select id="nhk-media-usage-role"><option value="featured_primary">featured_primary</option><option value="inline_primary">inline_primary</option><option value="inline_supporting">inline_supporting</option><option value="representative">representative</option><option value="evidence">evidence</option><option value="technical_detail">technical_detail</option></select><label for="nhk-media-usage-id">Usage UUID (replace/remove)</label><input id="nhk-media-usage-id"><label for="nhk-media-usage-revision">Usage revision (replace/remove)</label><input id="nhk-media-usage-revision" type="number" min="1"><button class="button button-primary" type="submit">Gửi qua Governance</button></form><pre id="nhk-media-usage-result" class="nhk-admin-state" aria-live="polite">Chưa có thao tác.</pre></section>';
