...
final class ProjectionAdminApi
{
    ...
    {
        try { $body = $request->get_json_params(); $body = is_array($body) ? $body : []; $revision = $this->projections->rebuild(new NodeReference((string) ($body['node_type'] ?? ''), (string) $request['node']), (string) ($body['canonical_url'] ?? ''), (string) ($body['h1'] ?? ''), ['per_section' => min(100, max(1, (int) ($body['per_section'] ?? 50)))]); return $revision->toArray(); } catch (\Throwable $error) { return new \WP_Error('nhk_projection_rebuild_failed', $error->getMessage(), ['status' => 400]); }
    }
    private function validate(\WP_REST_Request $request): array|\WP_Error { try { return $this->projections->validate((string) $request['node'], (int) $request->get_param('revision'))->toArray(); } catch (\Throwable $error) { return new \WP_Error('nhk_projection_validation_failed', $error->getMessage(), ['status' => 400]); } }
    private function publish(\WP_REST_Request $request): array|\WP_Error { try { return $this->projections->publish((string) $request['node'], (int) $request->get_param('revision'))->toArray(); } catch (\Throwable $error) { return new \WP_Error('nhk_projection_publish_failed', $error->getMessage(), ['status' => 400]); } }
    private function discard(\WP_REST_Request $request): array|\WP_Error { try { $this->projections->discard((string) $request['node'], (int) $request->get_param('revision')); return ['status' => 'discarded']; } catch (\Throwable $error) { return new \WP_Error('nhk_projection_discard_failed', $error->getMessage(), ['status' => 400]); } }
