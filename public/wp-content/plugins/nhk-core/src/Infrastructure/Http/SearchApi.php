...
final class SearchApi
{
    ...
    private function search(\WP_REST_Request $request): array|\WP_Error
    {
        ...
        $groups['media'] = !$this->status || $this->status->mediaStorageReady() ? array_map($this->media(...), array_values(array_filter($this->media->list(), fn (Media $item): bool => $item->active && $this->matches($term, $item->canonicalName, $item->stableKey)))) : [];
        $groups['videos'] = !$this->status || $this->status->videoStorageReady() ? array_map($this->video(...), array_values(array_filter($this->videos->list(), fn (Video $item): bool => $item->active && $this->matches($term, $item->title, $item->externalVideoId, $item->canonicalUrl)))) : [];
