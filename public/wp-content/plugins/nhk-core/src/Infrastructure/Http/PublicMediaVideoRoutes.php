...
final class PublicMediaVideoRoutes
{
    ...
    public function template(string $template): string { $videoKey = (string) get_query_var('nhk_video_key'); $videoSlug = (string) get_query_var('nhk_video_slug'); if ($videoKey !== '' || $videoSlug !== '' || get_query_var('nhk_video_page') !== '') { if ($videoKey !== '' || $videoSlug !== '') { $video = $videoSlug !== '' ? $this->query->videoBySlug(rawurldecode($videoSlug)) : $this->query->videoDetail(rawurldecode($videoKey)); if ($video === null) return $this->notFound(); if ($videoKey !== '') { wp_safe_redirect(home_url((string) ($video['public_url'] ?? '/video/')), 301, 'NHK canonical video route'); exit; } $path = (string) ($video['public_url'] ?? ''); $GLOBALS['nhk_core_video_context'] = ['mode' => 'detail', 'video' => $video, 'seo_projection' => $this->seo($path)]; } else { $path = '/video/'; $GLOBALS['nhk_core_video_context'] = ['mode' => 'archive', 'archive' => $this->query->videoArchive(max(1, (int) get_query_var('nhk_video_page', 1))), 'seo_projection' => $this->seo($path)]; } $found = locate_template('video.php'); return $found !== '' ? $found : $template; } $mediaRoute = (string) get_query_var('nhk_media_route'); if ($mediaRoute === '') return $template; if ((string) get_query_var('nhk_media_slug') !== '') return $this->notFound(); $path = '/thu-vien/'; $GLOBALS['nhk_core_media_context'] = ['mode' => 'archive', 'archive' => $this->query->mediaArchive(max(1, (int) get_query_var('nhk_media_page', 1))), 'seo_projection' => $this->seo($path)]; $found = locate_template('media.php'); return $found !== '' ? $found : $template; }
    private function seo(string $path): array { return (new PublicSeoProjection())->project(['path' => $path, 'eligible' => true, 'readiness' => 'READY', 'canonical_url' => $path, 'public_eligible' => true], ['type' => 'CollectionPage']); }
