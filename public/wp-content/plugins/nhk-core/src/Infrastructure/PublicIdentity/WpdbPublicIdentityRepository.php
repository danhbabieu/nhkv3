...
final class WpdbPublicIdentityRepository implements PublicIdentityRepository
{
    ...
    {
        $row = $this->wpdb->get_var($this->wpdb->prepare('SELECT id FROM '.$this->historyTable().' WHERE route_type=%s AND route_path=%s LIMIT 1', $historicRoute->routeType, $historicRoute->path));
        if ($row !== null) return PublicIdentityMutationResult::rejected(PublicIdentityMutationResult::CONFLICT);
        $ok = $this->wpdb->query($this->wpdb->prepare('INSERT INTO '.$this->historyTable().' (identity_uuid,route_type,route_path,old_slug,revision,created_at) VALUES (%s,%s,%s,%s,%d,%s)', UuidCodec::toBinary($historicRoute->identityId),$historicRoute->routeType,$historicRoute->path,$historicRoute->oldSlug,$historicRoute->replacementRevision,$historicRoute->createdAt ?? gmdate('Y-m-d H:i:s.u')));
