...
final class WpdbPublicIdentityRepository implements PublicIdentityRepository
{
    ...
    public function allocate(array $r, string $key): array
    {
        ...
        $ok = $this->wpdb->query($this->wpdb->prepare('INSERT INTO '.$this->currentTable().' (identity_uuid,owner_kind,owner_uuid,route_type,current_slug,current_path,collision_scope,route_policy_version,revision,idempotency_key,created_at,updated_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,1,%s,%s,%s)', UuidCodec::toBinary($id),$r['owner_kind'],UuidCodec::toBinary($r['owner_id']),$r['route_type'],$r['current_slug'],$r['current_path'],$r['collision_scope'],$r['route_policy_version'],$key,$now,$now));
    ...
    public function change(array $r, string $oldPath, int $expectedRevision, string $key): array
    {
        ...
        $ok=$this->wpdb->query($this->wpdb->prepare('UPDATE '.$this->currentTable().' SET current_slug=%s,current_path=%s,revision=revision+1,idempotency_key=%s,updated_at=%s WHERE identity_uuid=%s AND revision=%d',$r['current_slug'],$r['current_path'],$key,$now,UuidCodec::toBinary((string)$r['identity_id']),$expectedRevision));
        if ($ok !== 1) { $this->wpdb->query('ROLLBACK'); throw new \RuntimeException('STALE_REVISION'); }
        $historyOk=$this->wpdb->query($this->wpdb->prepare('INSERT INTO '.$this->historyTable().' (identity_uuid,route_type,collision_scope,route_path,old_slug,revision,created_at) VALUES (%s,%s,%s,%s,%s,%d,%s)',UuidCodec::toBinary((string)$r['identity_id']),$current['route_type'],$current['collision_scope'],$oldPath,$current['current_slug'],$expectedRevision,$now));
    ...
    public function resolveHistoric(string $routeType, string $scope, string $path): PublicIdentityMutationResult { return $this->resolveExact($routeType, $scope, $path); }
    public function resolveExact(string $routeType, string $scope, string $path): PublicIdentityMutationResult { $rows=$this->wpdb->get_results($this->wpdb->prepare('SELECT h.identity_uuid AS historic_identity_uuid,h.route_type AS historic_route_type,h.collision_scope AS historic_collision_scope,h.route_path AS historic_path,h.old_slug AS historic_old_slug,h.revision AS historic_revision,h.created_at AS historic_created_at,i.identity_uuid AS current_identity_uuid,i.owner_kind AS current_owner_kind,i.owner_uuid AS current_owner_uuid,i.route_type AS current_route_type,i.current_slug AS current_slug,i.current_path AS current_path,i.collision_scope AS current_collision_scope,i.route_policy_version AS current_policy_version,i.revision AS current_revision,i.created_at AS current_created_at,i.updated_at AS current_updated_at FROM '.$this->historyTable().' h LEFT JOIN '.$this->currentTable().' i ON i.identity_uuid=h.identity_uuid WHERE h.route_type=%s AND h.collision_scope=%s AND h.route_path=%s',$routeType,$scope,$path),ARRAY_A)?:[]; if(count($rows)!==1)return PublicIdentityMutationResult::rejected(count($rows)>1?PublicIdentityMutationResult::AMBIGUOUS_HISTORY:PublicIdentityMutationResult::UNKNOWN_ROUTE); $r=$rows[0]; if(!isset($r['current_identity_uuid']) || !isset($r['current_path']))return PublicIdentityMutationResult::rejected(PublicIdentityMutationResult::CONFLICT); $identity=new PublicIdentity(UuidCodec::fromBinary((string)$r['current_identity_uuid'],), (string)$r['current_owner_kind'], UuidCodec::fromBinary((string)$r['current_owner_uuid']), (string)$r['current_route_type'], (string)$r['current_slug'], (string)$r['current_collision_scope'], (string)$r['current_policy_version'], (int)$r['current_revision'], $r['current_created_at']??null, $r['current_updated_at']??null, (string)$r['current_path']); $history=new HistoricPublicRoute(UuidCodec::fromBinary((string)$r['historic_identity_uuid']), (string)$r['historic_route_type'], (string)$r['historic_collision_scope'], (string)$r['historic_path'], (string)$r['historic_old_slug'], (int)$r['historic_revision'], $r['historic_created_at']??null); return PublicIdentityMutationResult::accepted($identity,$history); }
    public function resolvePath(string $path): PublicIdentityMutationResult { $rows=$this->wpdb->get_results($this->wpdb->prepare('SELECT route_type,collision_scope FROM '.$this->historyTable().' WHERE route_path=%s',$path),ARRAY_A)?:[]; if(count($rows)!==1)return PublicIdentityMutationResult::rejected(count($rows)>1?PublicIdentityMutationResult::AMBIGUOUS_HISTORY:PublicIdentityMutationResult::UNKNOWN_ROUTE); return $this->resolveExact((string)$rows[0]['route_type'],(string)$rows[0]['collision_scope'],$path); }
    ...
    private function hydrate(array $r):array { return ['identity_id'=>UuidCodec::fromBinary((string)$r['identity_uuid']),'owner_kind'=>(string)$r['owner_kind'],'owner_id'=>UuidCodec::fromBinary((string)$r['owner_uuid']),'route_type'=>(string)$r['route_type'],'current_slug'=>(string)$r['current_slug'],'collision_scope'=>(string)$r['collision_scope'],'route_policy_version'=>(string)$r['route_policy_version'],'revision'=>(int)$r['revision'],'current_path'=>(string)($r['current_path'] ?? '')]; }
    private function toDomain(array $r): PublicIdentity { return new PublicIdentity((string)UuidCodec::fromBinary((string)$r['identity_uuid']),(string)$r['owner_kind'],(string)UuidCodec::fromBinary((string)$r['owner_uuid']),(string)$r['route_type'],(string)$r['current_slug'],(string)$r['collision_scope'],(string)$r['route_policy_version'],(int)$r['revision'],$r['created_at'] ?? null,$r['updated_at'] ?? null,$r['current_path'] ?? null); }
}
