...
final class ClaimProjectionService
{
    ...
    public function getPublishedSeoProjection(string $nodeUuid): ?array
    {
        ...
        return $published?->payload['seo'] ?? null;
    }
    ...
    public function getProjectionStatus(string $nodeUuid): array
    {
        ...
        return ['node_uuid' => $nodeUuid, 'published_revision' => $published?->revision, 'candidate_revision' => $candidate?->revision, 'status' => $candidate?->status ?? ($published?->status ?? 'missing'), 'dirty_sections' => $candidate?->dirtySections ?? [], 'claim_count' => (int) ($candidate?->payload['ledger']['claim_count'] ?? $published?->payload['ledger']['claim_count'] ?? 0), 'validation' => $candidate?->status === ProjectionStatus::READY ? 'ready' : ($candidate === null ? 'missing' : $candidate->status)];
    }
