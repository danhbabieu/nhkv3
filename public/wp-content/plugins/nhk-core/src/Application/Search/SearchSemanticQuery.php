...
final class SearchSemanticQuery
{
    ...
    public function extend(array $groups, string $term): array
    {
        $term = trim($term);
        ...
        if ($this->ready('knowledge')) foreach ($this->claims->list() as $item) if ($item->active && $this->matches($term, $item->claimText, $item->stableKey)) $groups['knowledge'][] = ['type' => 'knowledge', 'id' => $item->canonicalId, 'title' => $item->claimText, 'stable_key' => $item->stableKey];
        return $groups;
    }
