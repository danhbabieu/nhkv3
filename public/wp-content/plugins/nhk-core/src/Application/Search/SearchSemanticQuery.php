...
final class SearchSemanticQuery
{
    ...
    public function extend(array $groups, string $term, int $page = 1, int $perPage = 12): array
    {
        ...
        if ($this->ready('video')) foreach ($this->videos->list() as $item) if ($item->active && $item->hasValidPublicReference() && ($path = PublicRouteResolver::videoPath($item->title, $item->externalVideoId)) !== null && $this->matches($term, $item->title, $item->externalVideoId, $item->canonicalUrl)) $groups['videos'][] = ['type' => 'video', 'title' => $item->title ?: 'Video NHK', 'platform' => $item->platform, 'url' => home_url($path)];
