...
final class VideoRelationAdminService
{
    ...
    private function resolveEvidence(Video $video, string $targetType, string $targetId): Evidence
    {
        ...
        $source = $this->sources->findByStableKey($sourceKey); if ($source !== null) { if (!$source->active || $source->locator !== $video->canonicalUrl || $source->metadata !== $sourceMetadata) throw new \RuntimeException('WRONG_SOURCE_PROVENANCE'); } else $source = $this->knowledge->createSource($sourceKey, $sourceData['source_title'] ?: 'YouTube video source', 'website', $video->canonicalUrl, $sourceMetadata);
        ...
        $claim = $this->claims->findByStableKey($claimKey); if ($claim !== null) { if (!$claim->active || $claim->provenance !== $claimProvenance) throw new \RuntimeException('WRONG_CLAIM_PROVENANCE'); } else $claim = $this->knowledge->createClaim($claimKey, 'Video ' . $video->canonicalId . ' has a registered about relation to ' . $targetType . ' ' . $targetId . '.', 'provenance', $claimProvenance);
