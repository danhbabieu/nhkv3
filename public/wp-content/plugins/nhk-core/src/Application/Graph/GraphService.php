...
final class GraphService {
    ...
    public function findOutgoing(NodeReference $source, ?string $predicate=null, int $after=0, int $limit=50, bool $includeRetired=false): array { $ref=$this->endpoints->assertExists($source); $node=$this->repository->findNode($ref)??$this->repository->resolveNode($ref); return $this->repository->outgoing($node,$predicate,max(0,$after),min(200,max(1,$limit)),$includeRetired); }
    public function findIncoming(NodeReference $target, ?string $predicate=null, int $after=0, int $limit=50, bool $includeRetired=false): array { $ref=$this->endpoints->assertExists($target); return $this->repository->incoming($this->repository->resolveNode($ref),$predicate,max(0,$after),min(200,max(1,$limit)),$includeRetired); }
