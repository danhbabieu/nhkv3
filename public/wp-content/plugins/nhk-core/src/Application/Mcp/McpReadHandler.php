...
final class McpReadHandler
{
    ...
    private function publicAsset(MediaAsset $asset): array { $filename = is_string($asset->metadata['canonical_filename'] ?? null) ? $asset->metadata['canonical_filename'] : basename($asset->storageKey); return ['id' => $asset->assetId, 'kind' => $asset->kind, 'mime_type' => $asset->mimeType, 'byte_size' => $asset->byteSize, 'width' => $asset->width, 'height' => $asset->height, 'public_url' => (new \NHK\Core\Application\Media\PublicMediaAssetUrlResolver())->path($filename)]; }
