        ...
        return [
            ...
            self::tool('nhk.capture.ingest', 'Capture new editorial input or continue one existing Capture; classify intent before creating an Article, preserve canonical owners, reconcile typed MediaUsage bindings even without an Article, resolve bounded semantic context when required and return the current read-back.', [
                        ...
                        'relation_intents' => [
                            ...
                            'items' => [
                                ...
                                'properties' => [
                                    ...
                                    'target_type' => ['type' => 'string', 'minLength' => 1, 'maxLength' => 64],
                                    'target_uuid' => self::uuidField(),
                                    'provenance' => ['type' => 'string', 'minLength' => 1, 'maxLength' => 191],
                                    'reason' => ['type' => 'string', 'maxLength' => 2000],
                                ],
                                'required' => ['source_type', 'source_uuid', 'predicate', 'target_type', 'target_uuid'],
                                'additionalProperties' => false,
                            ],
                        ],
                        'relations' => [
                ...
                'media_operations' => ['type' => 'array', 'maxItems' => 20, 'items' => ['type' => 'object', 'properties' => [
                    'operation' => ['type' => 'string', 'enum' => ['update', 'add', 'replace', 'remove', 'representative_bind', 'set_featured']],
                    'media_ref' => ['type' => 'object', 'properties' => ['id' => self::uuidField(), 'stable_key' => ['type' => 'string'], 'attachment_id' => ['type' => 'integer', 'minimum' => 1], 'url' => ['type' => 'string', 'format' => 'uri']], 'additionalProperties' => false],
                    'media' => ['type' => 'object'],
                    'target' => ['type' => 'object', 'properties' => ['type' => ['type' => 'string'], 'id' => ['type' => 'string'], 'blog_id' => ['type' => 'integer', 'minimum' => 1], 'post_id' => ['type' => 'integer', 'minimum' => 1], 'stable_key' => ['type' => 'string'], 'url' => ['type' => 'string', 'format' => 'uri']], 'additionalProperties' => false],
            ...
            self::tool('nhk.article.publish.review', 'Evaluate one native WordPress Post and return PASS, OWNER_REVIEW_REQUIRED or SYSTEM_BLOCKED before publication.', ['post_id' => ['type' => 'integer', 'minimum' => 1], 'expected_state_token' => ['type' => 'string', 'pattern' => '^[a-fA-F0-9]{64}$'], 'idempotency_key' => ['type' => 'string', 'minLength' => 1], 'evidence' => ['type' => 'object']], ['post_id', 'expected_state_token', 'idempotency_key', 'evidence'], true),
            self::tool('nhk.article.publish.approve', 'Approve the exact eligible publication exception and continue controlled native publication.', ['post_id' => ['type' => 'integer', 'minimum' => 1], 'expected_state_token' => ['type' => 'string', 'pattern' => '^[a-fA-F0-9]{64}$'], 'idempotency_key' => ['type' => 'string', 'minLength' => 1], 'decision_id' => ['type' => 'string', 'minLength' => 1], 'affirmation' => ['type' => 'string', 'minLength' => 1], 'evidence' => ['type' => 'object']], ['post_id', 'expected_state_token', 'idempotency_key', 'decision_id', 'affirmation', 'evidence'], true),
            ...
            self::tool('nhk.media.ingest', 'Ingest governed Media metadata, or bind one already-uploaded WordPress image attachment into the canonical Media lifecycle without semantic inference.', [
                ...
                'max_width' => ['type' => 'integer', 'minimum' => 1, 'maximum' => \NHK\Core\Application\Media\PublicImageSizingPolicy::MAX_LONG_EDGE],
                'max_height' => ['type' => 'integer', 'minimum' => 1, 'maximum' => \NHK\Core\Application\Media\PublicImageSizingPolicy::MAX_LONG_EDGE],
                ...
                'intended_relations' => [
                    ...
                    'items' => [
                        ...
                        'properties' => [
                            ...
                            'evidence_refs' => ['type' => 'array', 'minItems' => 1, 'items' => ['type' => 'object', 'properties' => ['evidence_id' => self::uuidField()], 'required' => ['evidence_id'], 'additionalProperties' => false]],
                            'reason' => ['type' => 'string'],
                            'confidence' => ['type' => 'number', 'minimum' => 0, 'maximum' => 1],
                        ],
                        'required' => ['target_id', 'target_type', 'evidence_refs'],
                        'additionalProperties' => false,
                    ],
                ],
            ], ['url'], true),
