...
final class AuthorityService {
 ...
 private function mutate(string $id,int $revision,callable $fn,string $event):AuthorityEntity{$e=$this->repo->findByCanonicalId($id);if(!$e)throw new AuthorityRevisionConflict('Entity not found.');$n=$fn($e);if($n->canonicalName===$e->canonicalName&&$n->state===$e->state&&$n->retiredAt===$e->retiredAt)return $e;$n=$this->repo->update($n,$revision);$this->audit?->record($event,$n);return $n;}
