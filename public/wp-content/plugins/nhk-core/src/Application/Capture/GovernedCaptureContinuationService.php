    ...
    public function execute(string $captureId, string $continuationKey, array $context, array $control = []): array
    {
        ...
        if ($plans === []) {
            ...
            if (!$semanticDeltaRequested) {
                ...
                return ['status' => 'SKIPPED', 'plans' => [], 'writes' => [], 'reused_claims' => $reusedClaims, 'video_children' => [], 'blockers' => [], 'governance' => ['lifecycle' => [], 'status' => 'SKIPPED'], 'requirements' => $requirements, 'completion' => $this->completion->aggregateCapture($this->currentCaptureId, [], ['canonical_state' => 'COMPLETE', 'blockers' => [], 'skipped_requirements' => [$requirements['semantic_delta'] + ['name' => 'semantic_delta']]])];
            }
