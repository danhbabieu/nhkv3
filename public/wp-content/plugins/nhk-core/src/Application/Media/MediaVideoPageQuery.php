...
final class MediaVideoPageQuery
{
    ...
    public function videoBySlug(string $slug): ?array { if (!$this->available('video')) return null; $slug = trim($slug); $matches = array_values(array_filter($this->videos->list(), fn (Video $video): bool => $video->active && $video->hasValidPublicReference() && PublicRouteResolver::videoPath($video->title, $video->externalVideoId) === '/' . $slug . '/')); return count($matches) === 1 ? $this->video($matches[0]) : null; }
    ...
    private function video(Video $video): array
    {
        ...
        $publicUrl = PublicRouteResolver::videoPath((string) ($editorial['title'] ?? $video->title), $video->externalVideoId);
        $seoProjection = null;
        if ($sourceAvailable && ($source['availability'] ?? 'unknown') === 'available' && trim((string) ($editorial['title'] ?? '')) !== '' && trim((string) ($editorial['summary'] ?? '')) !== '' && is_array($metadata['semantic_attachments'] ?? null) && $metadata['semantic_attachments'] !== []) {
            $seoProjection = is_array($metadata['seo_projection'] ?? null) ? $metadata['seo_projection'] : (new \NHK\Core\Application\Video\VideoSeoProjection())->project(['source' => array_merge($source, ['external_video_id' => $video->externalVideoId]), 'editorial' => $editorial, 'seo' => is_array($metadata['seo'] ?? null) ? $metadata['seo'] : []], function_exists('home_url') ? home_url((string) $publicUrl) : (string) $publicUrl);
        }
        $result = [
