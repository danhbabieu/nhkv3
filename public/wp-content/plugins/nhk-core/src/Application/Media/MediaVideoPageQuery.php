...
final class MediaVideoPageQuery
{
    ...
    public function mediaDetail(string $id): ?array { if (!$this->available('media')) return null; $media = $this->media->findByCanonicalId($id); if (!$media || !$media->active) return null; return ['id' => $media->canonicalId, 'name' => $media->canonicalName, 'stable_key' => $media->stableKey, 'readiness' => $media->readiness, 'provenance' => $media->provenance, 'assets' => array_map($this->asset(...), $this->assets->listByMediaId($id)), 'usages' => array_map($this->usage(...), $this->usages->listByMediaId($id))]; }
