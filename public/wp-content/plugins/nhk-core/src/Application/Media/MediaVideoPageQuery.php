...
final class MediaVideoPageQuery
{
    ...
    public function mediaDetail(string $id): ?array { if (!$this->available('media')) return null; $media = $this->media->findByCanonicalId($id); if (!$media || !$media->active) return null; $assets = array_values(array_filter($this->assets->listByMediaId($id), static fn (MediaAsset $asset): bool => $asset->visibility === 'PUBLIC')); return ['id' => $media->canonicalId, 'name' => $media->canonicalName, 'stable_key' => $media->stableKey, 'readiness' => $media->readiness, 'assets' => array_map($this->asset(...), $assets), 'usages' => array_map($this->usage(...), $this->usages->listByMediaId($id))]; }
    public function videoDetail(string $id): ?array { if (!$this->available('video')) return null; $video = $this->videos->findByCanonicalId($id); return $video && $video->active ? ['id' => $video->canonicalId, 'title' => $video->title, 'platform' => $video->platform, 'external_id' => $video->externalVideoId, 'url' => $video->canonicalUrl] : null; }
