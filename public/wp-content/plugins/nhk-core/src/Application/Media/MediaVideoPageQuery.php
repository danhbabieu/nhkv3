...
final class MediaVideoPageQuery
{
    ...
    public function videoDetail(string $id): ?array { if (!$this->available('video') || !UuidCodec::isValid($id)) return null; $video = $this->videos->findByCanonicalId($id); $eligible = $video ? ($this->videoPolicy ?? new VideoUrlPolicy($this->identities))->project($video, new VideoPublicContextSelector())['eligible'] : false; return $video && $video->active && $video->hasValidPublicReference() && $eligible ? $this->video($video) : null; }
    public function videoBySlug(string $slug): ?array { if (!$this->available('video')) return null; $slug = trim($slug); $policy = $this->videoPolicy ?? new VideoUrlPolicy($this->identities); $selector = new VideoPublicContextSelector(); $matches = array_values(array_filter($this->videos->list(), fn (Video $video): bool => $video->active && ($result = $policy->project($video, $selector))['eligible'] && $result['path'] === '/' . $slug . '/')); return count($matches) === 1 ? $this->video($matches[0]) : null; }
    ...
    /** @return array{page:int,per_page:int,total:int,items:list<array<string,mixed>>} */
    public function videoArchive(int $page = 1, int $perPage = 12): array { $policy = $this->videoPolicy ?? new VideoUrlPolicy($this->identities); $selector = new VideoPublicContextSelector(); return $this->archive($this->available('video') ? $this->videos->list() : [], $page, $perPage, fn (Video $item): array => $this->video($item), static fn (object $item): bool => $item->active && $item->hasValidPublicReference() && $policy->project($item, $selector)['eligible']); }
    ...
    private function video(Video $video): array
    {
        ...
        if ($urlResult['eligible'] && $sourceAvailable && ($source['availability'] ?? 'unknown') === 'available') {
            $seoProjection = (new VideoSeoProjection())->project(['source' => array_merge($source, ['external_video_id' => $video->externalVideoId]), 'editorial' => $editorial, 'seo' => is_array($metadata['seo'] ?? null) ? $metadata['seo'] : []], function_exists('home_url') ? home_url((string) $publicUrl) : (string) $publicUrl);
        }
        $result = [
