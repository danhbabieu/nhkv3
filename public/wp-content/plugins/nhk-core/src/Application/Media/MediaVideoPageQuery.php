...
final class MediaVideoPageQuery
{
    ...
    public function videoDetail(string $id): ?array { if (!$this->available('video') || !UuidCodec::isValid($id)) return null; $video = $this->videos->findByCanonicalId($id); $url = $video ? ($this->videoPolicy ?? new VideoUrlPolicy($this->identities))->project($video, new VideoPublicContextSelector()) : null; return $video && $video->active && $video->hasValidPublicReference() && $url?->eligible ? $this->video($video, $url) : null; }
    public function videoBySlug(string $slug): ?array { if (!$this->available('video')) return null; $slug = trim($slug); $policy = $this->videoPolicy ?? new VideoUrlPolicy($this->identities); $selector = new VideoPublicContextSelector(); $matches = array_values(array_filter($this->videos->list(), fn (Video $video): bool => $video->active && ($result = $policy->project($video, $selector))->eligible && $result->finalPath === '/' . $slug . '/')); return count($matches) === 1 ? $this->video($matches[0], $policy->project($matches[0], $selector)) : null; }
    ...
    /** @return array{page:int,per_page:int,total:int,items:list<array<string,mixed>>} */
    public function videoArchive(int $page = 1, int $perPage = 12): array { $policy = $this->videoPolicy ?? new VideoUrlPolicy($this->identities); $selector = new VideoPublicContextSelector(); return $this->archive($this->available('video') ? $this->videos->list() : [], $page, $perPage, fn (Video $item): array => $this->video($item, $policy->project($item, $selector)), static fn (object $item): bool => $item->active && $item->hasValidPublicReference() && $policy->project($item, $selector)->eligible); }
    ...
    private function video(Video $video, ?PublicUrlResult $urlResult = null): array
    {
        ...
        $urlResult ??= ($this->videoPolicy ?? new VideoUrlPolicy($this->identities))->project($video, new VideoPublicContextSelector());
        $publicUrl = $urlResult->finalPath;
        $seoProjection = null;
        if ($urlResult->eligible && $sourceAvailable && ($source['availability'] ?? 'unknown') === 'available') {
            $seoProjection = (new VideoSeoProjection())->project(['source' => array_merge($source, ['external_video_id' => $video->externalVideoId]), 'editorial' => $editorial, 'seo' => is_array($metadata['seo'] ?? null) ? $metadata['seo'] : []], $urlResult);
        }
        $result = [
