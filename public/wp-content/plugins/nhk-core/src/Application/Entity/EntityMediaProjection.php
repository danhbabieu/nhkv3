...
final class EntityMediaProjection
{
    ...
    private function item(MediaUsage $usage): ?array
    {
        ...
        return ['media_id' => $media->canonicalId, 'asset_id' => $asset->assetId, 'stable_key' => $media->stableKey, 'url' => function_exists('home_url') ? (string) home_url($path) : $path, 'thumbnail_url' => $attachmentId > 0 && function_exists('wp_get_attachment_image_url') ? (string) (wp_get_attachment_image_url($attachmentId, 'medium') ?: (function_exists('home_url') ? home_url($path) : $path)) : (function_exists('home_url') ? (string) home_url($path) : $path), 'srcset' => $attachmentId > 0 && function_exists('wp_get_attachment_image_srcset') ? (string) wp_get_attachment_image_srcset($attachmentId, 'large') : null, 'sizes' => $attachmentId > 0 && function_exists('wp_get_attachment_image_sizes') ? (string) wp_get_attachment_image_sizes($attachmentId, 'large') : null, 'title' => $usage->title, 'alt' => $usage->altText, 'caption' => $usage->caption, 'width' => $asset->width, 'height' => $asset->height, 'role' => $usage->role, 'sort_order' => $usage->sortOrder];
    }
}
