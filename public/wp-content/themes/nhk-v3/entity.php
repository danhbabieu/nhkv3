...
?>
  <?php $breadcrumbItems = is_array($entity['navigation_breadcrumb'] ?? null) ? $entity['navigation_breadcrumb'] : (is_array($view['breadcrumbs'] ?? null) ? $view['breadcrumbs'] : []); if ($breadcrumbItems === []) $breadcrumbItems = [['name' => (string) ($identity['name'] ?? '')]]; get_template_part('template-parts/presentation/breadcrumbs', null, ['items' => array_merge([['label' => $label, 'url' => (string) ($context['archive_url'] ?? home_url($archivePaths[$type] ?? '/'))]], $breadcrumbItems)]); ?>
  ...
  <div class="semantic-layout">
    <div class="semantic-main">
      ...
      <?php if ($profileKey === 'clock_type' && is_array($entity['navigation_children'] ?? null) && $entity['navigation_children'] !== []): ?><section id="loai-con" class="dossier-section clock-type-navigation-children"><div class="section-head"><div><p class="eyebrow">LOẠI</p><h2>Nhóm con trực tiếp</h2></div></div><ul><?php foreach ($entity['navigation_children'] as $child): if (!is_array($child)) continue; $childUrl = nhk_v3_public_url($child['url'] ?? null); ?><li><?php if ($childUrl !== ''): ?><a href="<?php echo esc_url($childUrl); ?>"><?php endif; ?><?php echo esc_html((string) ($child['name'] ?? '')); ?><?php if ($childUrl !== ''): ?></a><?php endif; ?></li><?php endforeach; ?></ul></section><?php endif; ?>
      ...
      <section class="dossier-section relation-section relation-group-<?php echo esc_attr($group); ?>"><div class="section-head"><div><p class="eyebrow">Liên quan</p><h2><?php echo esc_html($heading); ?></h2></div></div>
        <?php if ($group === 'videos'): ?><div class="video-card-grid"><?php foreach ($items as $item): $url = nhk_v3_public_url($item['url'] ?? null); if ($url === '') continue; $thumb = trim((string) ($item['thumbnail_url'] ?? '')) ?: $fallback; $thumbMeta = is_array($item['thumbnail'] ?? null) ? $item['thumbnail'] : []; $orientation = nhk_v3_media_orientation_class($thumbMeta['width'] ?? 0, $thumbMeta['height'] ?? 0); ?><a class="video-card" href="<?php echo esc_url($url); ?>"><span class="visual-frame video-thumb <?php echo esc_attr($orientation); ?>"><img src="<?php echo esc_url($thumb); ?>" alt="" loading="lazy"<?php echo !empty($thumbMeta['width']) ? ' width="' . esc_attr((string) $thumbMeta['width']) . '"' : ''; ?><?php echo !empty($thumbMeta['height']) ? ' height="' . esc_attr((string) $thumbMeta['height']) . '"' : ''; ?>><span class="play-mark" aria-hidden="true">▶</span></span><span class="visual-card-body"><small><?php echo esc_html(($item['origin']['kind'] ?? '') === 'DIRECT' ? 'Liên quan trực tiếp' : 'Mở rộng từ quan hệ nền'); ?></small><strong><?php echo esc_html(nhk_v3_public_brand_text((string) ($item['title'] ?? ''))); ?></strong></span></a><?php endforeach; ?></div>
