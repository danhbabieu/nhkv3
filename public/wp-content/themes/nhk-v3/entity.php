...
?>
  <p class="breadcrumb"><a href="<?php echo esc_url(home_url('/')); ?>">NHK</a> <span>/</span> <a href="<?php echo esc_url((string) ($context['archive_url'] ?? home_url($archivePaths[$type] ?? '/'))); ?>"><?php echo esc_html($label); ?></a></p>
  ...
  <div class="semantic-layout">
    <div class="semantic-main">
      ...
      <?php if ($profileKey === 'clock_type' && ($profile['hierarchy']['status'] ?? '') === 'AVAILABLE'): $hierarchy = is_array($profile['hierarchy'] ?? null) ? $profile['hierarchy'] : []; $parent = is_array($hierarchy['parent'] ?? null) ? $hierarchy['parent'] : null; $children = is_array($hierarchy['children'] ?? null) ? $hierarchy['children'] : []; ?>
      <section id="phan-cap" class="dossier-section clock-type-hierarchy"><div class="section-head"><div><p class="eyebrow">Phân cấp</p><h2>Vị trí trong nhóm Loại đồng hồ</h2></div></div><?php if ($parent !== null): ?><p>Thuộc nhóm: <strong><?php echo esc_html((string) ($parent['name'] ?? '')); ?></strong></p><?php endif; ?><?php if ($children !== []): ?><h3>Các loại con</h3><ul><?php foreach ($children as $child): if (!is_array($child) || trim((string) ($child['name'] ?? '')) === '') continue; ?><li><?php echo esc_html((string) $child['name']); ?></li><?php endforeach; ?></ul><?php endif; ?><?php if ($parent === null && $children === []): ?><p>Chưa có quan hệ phân cấp được ghi nhận.</p><?php endif; ?></section>
      <?php endif; ?>
      ...
      <?php if ($hasLegacyAggregation || $hasClockTypeAggregation): $aggregationLabels = ['models' => 'Mẫu đồng hồ', 'variants' => 'Biến thể', 'movements' => 'Bộ máy', 'music' => 'Bản nhạc', 'components' => 'Linh kiện', 'classifications' => 'Phân loại', 'clock_types' => 'Loại đồng hồ', 'specimens' => 'Hiện vật', 'products' => 'Sản phẩm']; ?>
...
<?php elseif (is_array($context) && is_array($context['archive'] ?? null)): $archive = $context['archive']; $archiveItems = array_values(array_filter((array) ($archive['items'] ?? []), static fn($item): bool => is_array($item) && nhk_v3_public_url($item['url'] ?? null) !== '')); $archiveUrl = (string) ($context['archive_url'] ?? home_url($archivePaths[$type] ?? '/')); $archiveLabel = $profileKey === 'clock_type' ? 'Loại đồng hồ' : $label; ?>
  ...
  <?php if (($archive['available'] ?? true) === false): ?><div class="empty"><h2>Dữ liệu chưa sẵn sàng</h2><p>Kho hồ sơ hiện không thể truy vấn.</p></div><?php elseif ($archiveItems !== []): ?><div class="entity-grid visual-entity-grid"><?php foreach ($archiveItems as $item): $url = nhk_v3_public_url($item['url'] ?? null); $image = trim((string) ($item['media']['representative']['url'] ?? '')) ?: $fallback; $cardLabel = trim((string) ($item['profile_label'] ?? '')) ?: $label; ?><article class="entity-card visual-entity-card"><a class="entity-card-image" href="<?php echo esc_url($url); ?>"><img src="<?php echo esc_url($image); ?>" alt="<?php echo esc_attr((string) ($item['media']['representative']['alt'] ?? '')); ?>" loading="lazy"></a><div class="entity-card-body"><p class="eyebrow"><?php echo esc_html($cardLabel); ?></p><h2><a href="<?php echo esc_url($url); ?>"><?php echo esc_html(nhk_v3_public_brand_text((string) ($item['name'] ?? ''))); ?></a></h2></div></article><?php endforeach; ?></div><?php else: ?><div class="empty"><h2>Chưa có hồ sơ phù hợp</h2><p>Thử từ khóa khác hoặc quay lại sau khi hồ sơ được bổ sung.</p></div><?php endif; ?>
