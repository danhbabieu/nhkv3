...
<?php elseif (is_array($context) && is_array($context['archive'] ?? null)): $archive = $context['archive']; $archiveItems = array_values(array_filter((array) ($archive['items'] ?? []), static fn($item): bool => is_array($item) && nhk_v3_public_url($item['url'] ?? null) !== '')); $archiveUrl = (string) ($context['archive_url'] ?? home_url($archivePaths[$type] ?? '/')); $archiveLabel = $profileKey === 'clock_type' ? 'Nhóm đồng hồ' : $label; ?>
