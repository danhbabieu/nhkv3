...
final class NHK_V3_Home_Page_Query
{
    ...
    public function read(): array
    {
        ...
        foreach ($latest as $post) {
            $thumbnailId = has_post_thumbnail($post) ? (int) get_post_thumbnail_id($post) : 0;
            $image = $thumbnailId > 0 ? wp_get_attachment_image_src($thumbnailId, 'medium_large') : false;
            $width = is_array($image) ? (int) ($image[1] ?? 0) : 0; $height = is_array($image) ? (int) ($image[2] ?? 0) : 0;
            $articleItem = ['type' => 'article', 'label' => 'Bài viết', 'title' => get_the_title($post), 'url' => get_permalink($post), 'timestamp' => get_post_time('c', true, $post), 'created_at' => get_post_time('c', true, $post), 'summary' => wp_trim_words(wp_strip_all_tags((string) ($post->post_excerpt ?: $post->post_content)), 24), 'image_url' => is_array($image) ? (string) ($image[0] ?? '') : null, 'attachment_id' => $thumbnailId ?: null, 'image_srcset' => $thumbnailId > 0 && function_exists('wp_get_attachment_image_srcset') ? (string) wp_get_attachment_image_srcset($thumbnailId, 'medium_large') : '', 'image_sizes' => $thumbnailId > 0 && function_exists('wp_get_attachment_image_sizes') ? (string) wp_get_attachment_image_sizes($thumbnailId, 'medium_large') : '', 'orientation' => $height > $width ? 'portrait' : ($height === $width && $width > 0 ? 'square' : ($width > 0 ? 'landscape' : 'unknown')), 'width' => $width ?: null, 'height' => $height ?: null, 'tie_breaker' => 'post:' . (string) $post->ID];
            if (class_exists('NHK\\Core\\Application\\Presentation\\HomepageVisualPolicy')) $articleItem = (new \NHK\Core\Application\Presentation\HomepageVisualPolicy())->resolve($articleItem);
            $latestFeed[] = $articleItem;
        }
        if (class_exists('NHK\\Core\\Application\\Presentation\\LatestFirstOrder')) $latestFeed = \NHK\Core\Application\Presentation\LatestFirstOrder::sort($latestFeed, static fn (array $item): ?string => (string) ($item['timestamp'] ?? ''), static fn (array $item): ?string => (string) ($item['created_at'] ?? ''), static fn (array $item): string => (string) ($item['tie_breaker'] ?? ''));
        else usort($latestFeed, static function (array $a, array $b): int { $at = strtotime((string) ($a['timestamp'] ?? '')) ?: PHP_INT_MIN; $bt = strtotime((string) ($b['timestamp'] ?? '')) ?: PHP_INT_MIN; $ac = strtotime((string) ($a['created_at'] ?? '')) ?: PHP_INT_MIN; $bc = strtotime((string) ($b['created_at'] ?? '')) ?: PHP_INT_MIN; return $bt <=> $at ?: $bc <=> $ac ?: strcmp((string) ($b['tie_breaker'] ?? ''), (string) ($a['tie_breaker'] ?? '')); });
        foreach ($latestFeed as &$item) {
            unset($item['tie_breaker'], $item['created_at']);
        }
