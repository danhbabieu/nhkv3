...
?>
        ...
        <div class="nhk-album" data-nhk-album tabindex="0" aria-label="Album ảnh trong bài viết">
          <div class="album-stage">
            <?php foreach ($galleryImages as $index => $item):
              ...
              $fullUrl = trim((string) ($item['url'] ?? $item['image_url'] ?? ''));
              $thumbnailUrl = trim((string) ($item['thumbnail_url'] ?? '')) ?: $fullUrl;
            ?>
  ...
  <?php if ($postDossier !== null): foreach ($relationLabels as $group => $heading): $items = is_array($relationSections[$group] ?? null) ? $relationSections[$group] : []; if ($items === []) continue; ?>
    <section class="post-related relation-group-<?php echo esc_attr($group); ?>">
      ...
      <?php if ($group === 'videos'): ?><div class="video-card-grid"><?php foreach ($items as $item): $url = nhk_v3_public_url($item['url'] ?? null); if ($url === '') continue; $thumb = trim((string) ($item['thumbnail_url'] ?? '')) ?: $fallback; $thumbMeta = is_array($item['thumbnail'] ?? null) ? $item['thumbnail'] : []; $orientation = nhk_v3_media_orientation_class($thumbMeta['width'] ?? 0, $thumbMeta['height'] ?? 0); ?><a class="video-card" href="<?php echo esc_url($url); ?>"><span class="visual-frame video-thumb <?php echo esc_attr($orientation); ?>"><img src="<?php echo esc_url($thumb); ?>" alt="" loading="lazy"<?php echo !empty($thumbMeta['width']) ? ' width="' . esc_attr((string) $thumbMeta['width']) . '"' : ''; ?><?php echo !empty($thumbMeta['height']) ? ' height="' . esc_attr((string) $thumbMeta['height']) . '"' : ''; ?>><span class="play-mark" aria-hidden="true">▶</span></span><span class="visual-card-body"><small><?php echo esc_html(($item['origin']['kind'] ?? '') === 'DIRECT' ? 'Liên quan trực tiếp' : 'Mở rộng từ quan hệ nền'); ?></small><strong><?php echo esc_html((string) ($item['title'] ?? '')); ?></strong></span></a><?php endforeach; ?></div>
      <?php elseif ($group === 'media'): ?><div class="media-mosaic related-media-grid"><?php foreach ($items as $item): $image = trim((string) ($item['image_url'] ?? '')) ?: $fallback; $mediaDestination = nhk_v3_public_url($item['article_url'] ?? $item['url'] ?? $item['image_url'] ?? null); if ($mediaDestination === '') continue; ?><figure class="media-figure"><a href="<?php echo esc_url($mediaDestination); ?>"><img src="<?php echo esc_url($image); ?>" alt="<?php echo esc_attr((string) ($item['alt'] ?? $item['title'] ?? '')); ?>" loading="lazy"></a><figcaption><?php echo esc_html((string) ($item['title'] ?? 'Hình ảnh')); ?> · <?php echo esc_html(($item['origin']['kind'] ?? '') === 'DIRECT' ? 'liên quan trực tiếp' : 'liên quan mở rộng'); ?></figcaption></figure><?php endforeach; ?></div>
