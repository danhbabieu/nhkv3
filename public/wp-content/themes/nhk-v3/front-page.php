...
<main id="main-content" class="site-main home-page-v2">
  <section class="hero home-hero-v2">
    ...
    <div class="hero-visual" aria-label="Ảnh nổi bật từ kho NHK">
      <?php if ($heroMedia !== [] && trim((string) ($heroMedia[0]['image_url'] ?? '')) !== '' && ($heroMedia[0]['has_real_image'] ?? false) === true): $item = $heroMedia[0]; $dimensions = nhk_v3_media_dimensions($item); $orientation = nhk_v3_media_orientation_class($dimensions['width'], $dimensions['height']); $alt = trim((string) ($item['alt'] ?? $item['title'] ?? 'Ảnh tư liệu NHK')); ?>
        <figure class="hero-image <?php echo esc_attr($orientation); ?>"><span class="hero-image-frame"><img src="<?php echo esc_url((string) $item['image_url']); ?>" alt="<?php echo esc_attr($alt); ?>" width="<?php echo esc_attr((string) max(1, $dimensions['width'])); ?>" height="<?php echo esc_attr((string) max(1, $dimensions['height'])); ?>"<?php if (trim((string) ($item['srcset'] ?? '')) !== ''): ?> srcset="<?php echo esc_attr((string) $item['srcset']); ?>"<?php endif; ?><?php if (trim((string) ($item['sizes'] ?? '')) !== ''): ?> sizes="<?php echo esc_attr((string) ($item['sizes'])); ?>"<?php endif; ?> loading="eager" fetchpriority="high"></span><figcaption><?php echo esc_html((string) ($item['title'] ?? 'Ảnh tư liệu NHK')); ?></figcaption></figure>
