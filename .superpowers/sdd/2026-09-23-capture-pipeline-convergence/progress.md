...
- Task 4: complete — implementation audit/review PASS with no product-code change. Existing Media/MediaUsage convergence already preserves physical Media identity, endpoint/role/placement usage identity, desired-state reconciliation, optimistic retry/idempotency and canonical read-back/native projection agreement. Focused media suite passed 61 tests / 209 assertions; no staging/live mutation. Six-intent review: IMAGE_ARTICLE media required path and TEXT_ARTICLE optional path remain distinct; VIDEO thumbnail remains Video-owned; MEDIA_ENRICHMENT remains typed MediaUsage-only; KNOWLEDGE_DELTA/KNOWLEDGE_REPAIR remain non-Article semantic branches.
- Task 5: complete — implementation audit/review PASS with no product-code change. Existing Article ingest normalization/read-back satisfies aliases, explicit native field precedence, allowlisted fields, CAS/idempotency and mismatch diagnostics. Focused Article suite passed 29 tests / 120 assertions; no staging/live mutation. Six-intent review: Article field mutation remains limited to IMAGE_ARTICLE/TEXT_ARTICLE; VIDEO uses Video owner fields; MEDIA_ENRICHMENT/KNOWLEDGE_DELTA/KNOWLEDGE_REPAIR do not enter Article field writes.
- Task 6: complete — implementation audit/review PASS with no product-code change. Existing retry/state boundaries rehydrate canonical Article state and token, reject stale CAS, rotate token after MediaUsage/publication read-back and preserve append-only receipt history. Focused state suite passed 53 tests / 256 assertions; no staging/live mutation. Six-intent review: Article token flow remains IMAGE/TEXT scoped; VIDEO completion retry does not re-enter Article work; MEDIA_ENRICHMENT and both Knowledge intents remain non-Article.
- Task 7: complete — implementation audit/review PASS with no product-code change. Existing publication/completion/route boundaries derive current-evidence blockers, require concrete owner IDs and verify native route/public read-back. Focused suite passed 70 tests / 321 assertions; no staging/live mutation. Six-intent review: IMAGE_ARTICLE required Media and TEXT_ARTICLE optional Media remain distinct; VIDEO public owner remains Video-scoped; MEDIA_ENRICHMENT/KNOWLEDGE_DELTA/KNOWLEDGE_REPAIR do not acquire Article publication requirements.
- Task 8: complete — full regression/controller review PASS after removing unrelated hierarchical-subject scratch code that had been accidentally included in an earlier local commit, and moving the canonical execution-state title to the snapshot header. Full NHK Unit passed 2,313 tests / 13,500 assertions with 18 warnings, 39 deprecations and 29 PHPUnit deprecations; NHK Contract passed 6 tests / 48 assertions. No integration/staging mutation. Six-intent matrix remains covered by the accumulated focused suites; no hard-coded reproduction identity was added.
- Task 9: complete — documentation review PASS. Existing normative Article/Media/MCP/public URL contracts already cover the verified behavior; only execution-state evidence was updated. No Constitution or contract vocabulary change was needed. No staging/live mutation.
- Task 10: complete locally / staging UNVERIFIED — fail-closed review PASS. Runtime identity, staging endpoint/credentials, exact signed `STAGING_ACCEPTANCE_SCOPE`, `NHK_WP_TEST_PATH` and `NHK_WP_TEST_DB` were all unset, so no staging E2E or semantic mutation was attempted. Final local Unit passed 2,313 tests / 13,500 assertions; Contract passed 6 tests / 48 assertions; PHP lint and `git diff --check` passed. Build/lint beyond PHP: `UNVERIFIED`.
- Task 3: pending.
- Task 4: pending.
- Task 5: pending.
- Task 6: pending.
