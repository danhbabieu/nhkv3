...
| Authority detail | `/{type}/{stable-key}/` and UUID detail | `EntityPageQuery` → `entity.php` | Stable-key/UUID lookup, semantic facts, Graph-related groups | HTTP smoke and desktop/mobile detail visual QA pass for a real active Brand; fresh 41-route HTTP sweep passed real active Brand, Model, Movement, Music, Component, Variant and Classification stable-key details; Variant and Classification detail DOM checks at 390px/844px report no overflow, broken image, dead link, internal terminology or console error; 390px/768px route metrics pass |
...
| Media detail | `/media/{uuid}/` | `MediaVideoPageQuery` → `media.php` | Active, ready-only detail with readiness-aware reader-safe asset facts and usage facts | Active local Media detail 200 and desktop/mobile visual smoke pass; draft/non-ready Media now fails closed; image assets receive reader-safe `/media/asset/{uuid}/` URLs and render lazily; theme query omits provenance, storage/checksum/visibility/metadata and Graph endpoint identifiers; PRIVATE/draft or missing-source asset state renders an honest empty state; 390px/768px route metrics pass; asset policy remains open |
...
| Video detail | `/video/{uuid}/` | `MediaVideoPageQuery` → `video.php` | YouTube privacy embed only for validated 11-char IDs; reader-facing external-reference fields and approved display metadata only | Route contract and source-level embed validation pass; persisted internal metadata is omitted from theme detail; active-record browser coverage pending |
