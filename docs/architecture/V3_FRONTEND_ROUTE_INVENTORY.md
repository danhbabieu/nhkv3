...
| Video detail | `/video/{uuid}/` | `MediaVideoPageQuery` → `video.php` | YouTube privacy embed only for validated 11-char IDs; reader-facing external-reference fields only | Route contract and source-level embed validation pass; persisted metadata is omitted from theme detail; active-record browser coverage pending |
