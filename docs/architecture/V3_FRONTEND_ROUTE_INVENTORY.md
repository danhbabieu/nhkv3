...
| Media detail | `/media/{uuid}/` | `MediaVideoPageQuery` → `media.php` | Readiness-aware reader-safe asset facts and usage facts | Active local Media detail 200 and desktop/mobile visual smoke pass; theme query omits provenance, storage/checksum/visibility/metadata and Graph endpoint identifiers; PRIVATE/draft asset state renders an honest empty state; 390px/768px route metrics pass; asset policy remains open |
