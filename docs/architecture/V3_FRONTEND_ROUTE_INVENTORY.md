...
| Knowledge archive/detail | `/knowledge/`, `/knowledge/page/{n}/`, `/knowledge/claim/{uuid}/` | `KnowledgePageQuery` → `PublicKnowledgeRoutes` → `knowledge.php` | Active claims only; evidence is shown only when its source is active, with reader-facing source title/type and locator fallback | HTTP smoke and Knowledge pagination visual checks pass; source filtering and source presentation are unit-tested; public provenance policy remains open |
| Media archive | `/thu-vien/`, `/media/`, `/media/page/{n}/` | `MediaVideoPageQuery` → `PublicMediaVideoRoutes` → `media.php` | Active-only archive, honest empty state and bounded page links from query totals | Template contract now covers archive pagination; HTTP smoke and desktop archive visual QA pass; 390px/768px route metrics pass and `/media/page/2/` mobile screenshot pass; asset policy remains open |
...
| Video archive | `/video/`, `/video/page/{n}/` | `MediaVideoPageQuery` → `PublicMediaVideoRoutes` → `video.php` | Active-only external references, empty state and bounded page links from query totals | Template contract now covers archive pagination; HTTP smoke and desktop empty-state visual QA pass; 390px/768px route metrics pass and `/video/page/2/` mobile empty-state screenshot pass; active Video detail unavailable in local data |
