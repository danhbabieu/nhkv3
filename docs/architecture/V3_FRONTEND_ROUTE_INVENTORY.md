...
| Media archive | `/thu-vien/`, `/media/`, `/media/page/{n}/` | `MediaVideoPageQuery` → `PublicMediaVideoRoutes` → `media.php` | Active-only archive, honest empty state | HTTP smoke and desktop archive visual QA pass; 390px/768px route metrics pass and `/media/page/2/` mobile screenshot pass; gallery/pagination coverage beyond the inspected state remains |
...
| Video archive | `/video/`, `/video/page/{n}/` | `MediaVideoPageQuery` → `PublicMediaVideoRoutes` → `video.php` | Active-only external references and empty state | HTTP smoke and desktop empty-state visual QA pass; 390px/768px route metrics pass and `/video/page/2/` mobile empty-state screenshot pass; active Video detail unavailable in local data |
...
| 404 / pagination | Theme 404 and route-level page links | Theme templates | Empty states and bounded pagination are implemented | Core 404/route smoke and desktop 404 visual QA pass; all declared page-2 routes and empty/404 states have 390px/768px metrics with no overflow, custom archive page-two states emit `noindex,follow`, and `/model/page/2/`, `/component/page/2/`, `/media/page/2/`, `/video/page/2/`, `/knowledge/page/2/` and 404 visual checks pass; broader screenshot QA pending |
...
pass, but the latest shell retry is blocked by the current local MySQL/Apache
connection state. Remaining evidence is intentionally bounded to
