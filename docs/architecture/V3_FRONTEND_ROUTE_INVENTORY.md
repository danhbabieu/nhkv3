...
| Video detail | `/video/{semantic-slug}-{external-video-id}/` | governed Video URL policy → `video.php` | YouTube privacy embed only for validated 11-char IDs; reader-facing external-reference fields and approved display metadata only | Semantic-context slug policy and canary are documented; persisted Public Identity/current slug/history and active-record browser coverage remain pending |
