...
| Video detail | `/video/{semantic-slug}/` | governed Video URL policy → `video.php` | YouTube privacy embed only for validated 11-char IDs; reader-facing external-reference fields and approved display metadata only | Semantic-only slug policy and canary are documented; external ID remains internal lookup/dedupe metadata; persisted Public Identity/current slug/history and active-record browser coverage remain pending |
