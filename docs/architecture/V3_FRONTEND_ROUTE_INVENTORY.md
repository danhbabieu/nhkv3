...
| Media archive | `/thu-vien/`, `/media/`, `/media/page/{n}/` | `MediaVideoPageQuery` → `PublicMediaVideoRoutes` → `media.php` | Active, ready-only archive, honest empty state and bounded page links from query totals | Template contract now covers archive pagination; HTTP smoke and desktop archive visual QA pass; 390px/768px route metrics and `/media/page/2/` mobile screenshot pass without overflow; asset policy remains open |
