...
| Authority archive | `/{type}/`, `/{type}/page/{n}/` for nine registered types | `EntityPageQuery` → `PublicEntityRoutes` → `entity.php` | Active-only pagination and type catalog | HTTP smoke and desktop archive visual QA pass; declared page-2 routes have 390px/768px metrics, long title/key wrapping guards are in the theme, and `/model/page/2/` mobile visual pass; broader pagination QA pending |
