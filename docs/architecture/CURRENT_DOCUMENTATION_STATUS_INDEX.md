...
| Entity Profile / Clock Type | `ENTITY_PROFILE_CLOCK_TYPE_CONTRACT.md` | Brand and Clock Type are independent profiles; Clock Type is `classification + family=clock_type`; PR2 now provides application-level profile registry/resolver and a read-only dossier seam. Legacy `clock-type` remains compatibility-read only; root-route consumer parity remains a PR3 gap. |
