...
| Media | `Media` identity separate from `MediaAsset`, `MediaUsage` and WP attachment; multipart batch is the primary byte transport | source-original retained private/protected; derivatives remain under same Media; checksum does not auto-merge identity; batch manifest is ordered/idempotent with per-item partial failure; caption/alt/filename observations may feed Dictionary candidates only |
