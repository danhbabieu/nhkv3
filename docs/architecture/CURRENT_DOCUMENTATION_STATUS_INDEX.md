...
| Entity Profile / Clock Type | `ENTITY_PROFILE_CLOCK_TYPE_CONTRACT.md` | Brand and Clock Type are independent profiles; Clock Type is `classification + family=clock_type`; PR3 adds a shared public-dossier adapter and read-only root-route collision/resolution foundation. Legacy `clock-type` remains compatibility-read only; current router/allocation/live root consumer parity remains separately gated. |
