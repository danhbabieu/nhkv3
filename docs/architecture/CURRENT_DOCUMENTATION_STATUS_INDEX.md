...
The existing URL maintenance lifecycle now supports an exact single-owner
scope: `nhk.public-url.audit(owner_id)` produces a fresh owner-only receipt,
and the internal/admin `nhk-v3/public-url-reproject` Ability requires that
same UUID plus explicit confirmation. A global audit blocker is not scoped
eligibility evidence; it neither authorizes a global reproject nor blocks a
clean owner. No live Public Identity was reprojected in this checkpoint.
