...
| P8 Admin + MCP operational layer | IN PROGRESS | NHK Admin console now includes semantic/Graph lookup and governed proposal composer; relation mutations, eligibility/Controlled Apply, semantic read/search APIs and capability-gated local Streamable HTTP MCP transport are wired; external client/deployment interoperability and runtime QA remain pending. |
| P9 Frontend/UI parity | IN PROGRESS | Responsive NHK editorial theme now includes HomePageQuery discovery, grouped semantic search, read-only Authority comparison, featured/latest/topics/semantic modules, canonical entity/related, Post Graph-derived related groups, Media gallery and Video archive/detail surfaces with SEO metadata; desktop visual inspection passes, responsive/tablet/mobile QA remains pending. |
| P10 V2 → V3 Data Migration | IN PROGRESS | Versioned V2 normalize/export, guarded backup/restore, Migration006 ledger, Migration007 Evidence metadata, Migration008 MediaAsset metadata/visibility, governed native-post/entity/Knowledge URL redirects and resumable apply are implemented; local-dev checkpoint imported 2,379 rows including field-level PRIVATE MediaAsset metadata, 367 Knowledge redirects, 370 entity redirects, 34 native-post redirects and Source/Evidence citation rows, while 28 explicitly classified URL candidates, MediaAsset delivery/privacy policy, projections and domain-targeted posts remain gated; V2 media usage inventory is zero. |
