...
| P11 Reconciliation + parity + cutover readiness | IN PROGRESS / NOT READY | `CUTOVER_READINESS_REPORT.md` records implemented evidence and explicit blockers; latest guarded suite is 93 tests/509 assertions (241 tests/1,415 assertions combined), all-nine-type core route smoke is 34/34, opt-in real Authority detail smoke is 41/41, and MCP wire smoke passes; Proposal optional target UUID validation including empty optional normalization, nullable optional UUID schema/runtime semantics, shared strict public UUID boundary across REST/MCP/theme, Graph-related content and case-compatible REST/rewrite routes, strict MCP argument/schema enforcement with linked Proposal fields, non-empty required strings, domain enum validation, domain-shaped stable-key patterns and URI validation with guarded malformed-input rejection, semantic `format=uuid` validation and recursively complete Media asset/usage packets including non-empty storage/MIME/endpoint keys, MediaAsset PRIVATE-by-default construction/hydration/schema, repository-wide fail-closed domain hydration, Media/Video fail-closed JSON hydration, Authority fail-closed payload/state/revision hydration, and Admin proposal visibility for state, direct dependencies, eligibility/block reason and Apply status including inline reason-code rendering are covered; read-only domain candidate audit covers all 742 skipped domain posts without automatic mapping; count/semantic/UI/logic reconciliation is still gated by V2 data and WordPress runtime. |
...
guarded suite is 94 tests / 512 assertions and the combined suite is 244 tests /
1,424 assertions. V2 Video migration and its dry-run now canonicalize supported
YouTube URL forms, require the canonical UUID boundary and record URL/external-ID
...
WPDB conversion. Latest combined verification is 236 tests / 1,357 assertions.

Graph endpoint resolvers now use the shared strict UUID codec for Authority,
Media, Video, Knowledge, Source and Evidence references, including nil UUID
rejection.

MCP `tools/list` schemas now declare the same canonical UUID shape for public
read, evidence-ingest and Proposal ID fields; runtime validation remains the
authoritative nil/malformed-ID guard.

MCP transport now enforces required fields, types, numeric bounds, UUID
patterns, `format=uuid`, nested item types and `additionalProperties=false`
before tool dispatch; invalid calls return JSON-RPC `-32602`, while Proposal
create declares its linked-target/dependency and binding fields. Latest
combined verification is 237 tests / 1,368 assertions.
