...
| P10 V2 → V3 Data Migration | IN PROGRESS | Versioned V2 normalize/export, guarded backup/restore, Migration006 ledger, Migration007 Evidence metadata, Migration008 MediaAsset metadata/visibility and resumable apply are implemented; local-dev checkpoint imported 1,608 rows including field-level PRIVATE MediaAsset metadata plus Source/Evidence citation rows and one safe URL no-op, while unmapped URLs/usages/projections and domain-targeted posts remain gated. |
