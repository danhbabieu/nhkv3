...
| P8 Admin + MCP operational layer | IN PROGRESS | NHK Admin console now includes semantic/Graph lookup and governed proposal composer with explicit label/id associations; relation mutations, eligibility/Controlled Apply, semantic read/search APIs, governed `nhk.media.ingest` and capability-gated local Streamable HTTP MCP transport are wired; external client/deployment interoperability and runtime QA remain pending. |
