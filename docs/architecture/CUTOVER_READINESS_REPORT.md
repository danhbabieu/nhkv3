...
`nhk.semantic.resolve` read tool; local wire evidence and external client
interoperability remain open.

The repeatable wire smoke has been extended to assert the 19-tool catalog and
the non-mutating `nhk.semantic.resolve` call; the local HTTP harness was
unavailable during this checkpoint, so this runtime evidence remains pending.
...
Knowledge theme detail lookup now applies the same guard before repository
conversion; invalid UUID-shaped claim keys resolve to an empty result.

Graph-related public content now also validates canonical entity/media/video
IDs before repository access and returns empty groups when Graph lookup fails.
...
REST routes and public rewrites now accept the same case-compatible canonical
UUID boundary.
...
representable under strict schema enforcement.
...
Governance Proposal and ApplyAttempt reads now also omit out-of-range persisted
