...
| Gate | Result |
|---|---|
| Unit tests | PASS — 98 tests, 555 assertions |
| Plugin PHP lint | PASS |
| Theme PHP lint | PASS |
| `git diff --check` | PASS at checkpoints |
| Guarded WordPress integration | PASS — `NHK_WP_TEST_PATH=public NHK_WP_TEST_DB=nhk_v3_test vendor/bin/phpunit --configuration phpunit.xml.dist`; 45 tests, 355 assertions; combined current suite is 143 tests, 910 assertions; governed MCP Media/Video/Knowledge/Source/Evidence ingest lifecycles, Media/Video REST reads, Streamable HTTP Accept validation and bounded readiness-filtered REST/MCP semantic search pagination are included |
