...
| Gate | Result |
|---|---|
| Unit tests | PASS — 90 tests, 483 assertions |
| Plugin PHP lint | PASS |
| Theme PHP lint | PASS |
| `git diff --check` | PASS at checkpoints |
| Guarded WordPress integration | PARTIAL — latest complete evidence remains `NHK_WP_TEST_PATH=public NHK_WP_TEST_DB=nhk_v3_test composer test` with 44 tests, 347 assertions and combined 134 tests, 827 assertions; the current rerun was blocked by the local MySQL service exiting before TCP connection, while the unit contract is current at 90 tests/483 assertions; governed MCP Media/Video/Knowledge/Source/Evidence ingest lifecycles, Video REST read, Streamable HTTP Accept validation and bounded REST/MCP semantic search pagination are included |
