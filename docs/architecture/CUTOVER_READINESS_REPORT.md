...
Latest verification: guarded WordPress suite 176 tests/1,144 assertions;
...
- Responsive archive/detail QA now confirms editorial/entity/media/knowledge
  card and footer links use the NHK palette instead of browser-default blue,
  with visible accent-secondary hover/focus states; stylesheet cache-busting is
  synchronized.
...
- Theme design tokens now have one NHK source; the legacy token/rule block was
  ...
  is synchronized to 1.1.6, and cache-busting browser verification confirms no
...
| Gate | Result |
|---|---|
| Unit tests | PASS — 119 tests, 740 assertions |
